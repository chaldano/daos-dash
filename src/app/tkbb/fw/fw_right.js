...
export function prepareZoneDetail(selectedRow, tableArray, matrixdata) {
  ...
  let sadrip = matrixdata.sadrip
  console.log("sadrip-all",sadrip)
  
  let tabkeys = Object.keys(tableArray[0])
  let contentKey = tabkeys[0]

  let selectedContent = selectedRow.find('td:eq(0)').text()

  // Suche ausgewählte Zeile in Tabelle      
  let rowContent = tab.getRowByIndex(contentKey, selectedContent, tableArray)

  let serviceKey = `${rowContent.source}-${rowContent.service}`
  var targets = []
  var sources = []
  ...
  console.log("sadrip-source",sources)
  console.log("sadrip-target",targets)

  var zones = []
  let sz = new daos.Zone(rowContent.source, 'sourcezone')
  let tz = new daos.Zone(matrixdata.zone, 'targetzone')
  zones.push(sz)
  zones.push(tz)
